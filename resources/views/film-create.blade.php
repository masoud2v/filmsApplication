...
@section('content')
    ...
    @endif
    <form method="post" class="card card-body film-form" action="{{ route('films.store') }}"
          enctype="multipart/form-data">
        {{ csrf_field() }}

        <h4 class="mb-4">{{ __('Create New Film') }}</h4>
        ...
        <div class="form-group row">
            <label class="col col-12 col-sm-3 col-form-label" for="film-release_date">Realease Date</label>
            <div class="col col-12 col-sm">
                @php
                    $filmReleaseDate = $film->release_date ? $film->release_date->format('Y-m-d') : null;
                @endphp
                <input type="date" name="release_date" id="film-release_date"
                       class="form-control{{ $errors->has('release_date') ? ' is-invalid' : '' }}"
                       autocomplete="off"
                       value="{{ old('realease_date', $filmReleaseDate) }}" required>
        ...
        <div class="form-group row">
            ...
            <div class="col col-12 col-sm">
                <div><input type="file" name="photo"
                            accept="image/jpeg,image/png"{{ !$film->photo ? ' required' : '' }}></div>
                @if($errors->has('photo'))
